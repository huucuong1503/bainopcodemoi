...
uncheck=False
kiemtra=False
model = load_model('/root/bainopcodemoi/keras_model.h5')
    ...
    try:
        while True:
            ...
            try:
                ...
                blur3 = cv2.GaussianBlur(ngabatu, (5, 5), 0)
                imgHSV3 = cv2.cvtColor(blur3, cv2.COLOR_BGR2HSV)
                ...
                maskbatu=cv2.inRange(imgHSV3, lower, upper)
                ...
                if distance<300:
                    ...
                    try:
                        ...
                        print(lm,arrmax3,arrmin3)

                        if (uncheck == True and dudoan == 3):
                            print('thang')
                            Control(0,40)
                        elif uncheck == True and lm==False and arrmax3==-1 and arrmin3==-1:
                            print('thang')
                            Control(0, 40)
                            dithang(0.2)

                            print(dudoan)
                        elif (uncheck==True and dudoan==1):
                            print('uhuhu')
                            print(arrmax)
                            Control(-25,40)
                        elif (uncheck==True and dudoan==0):
                            print(arrmax)
                            print('uhuhu')
                            print(dudoan)
                            Control(25,40)
                        elif (uncheck==True and dudoan==5):
                            print('queophai')
                            Control(25,100)

                        elif (uncheck==True and dudoan==4):
                            Control(-25,100)
                            print('queotrai')
                else:
                ...
                if dem <=7:
                    ...
                    image1 = Image.open('/root/bainopcodemoi/bienbao.png')
                ...
                if 60>=h>=40 and (255 in img4 or 255 in img5) and 60>=w>=40 :
                    if abs(w-h)<=5:
                        #print(h)
                        try:
                            cv2.imwrite('/root/bainopcodemoi/bienbao.png', k[y:y + h, x:x + w, :])
                            image1 = Image.open('/root/bainopcodemoi/bienbao.png')
                            ...
                            dudoan=list(prediction[0]).index(max(prediction[0]))
                            kiemtra=True
                            check=True
                            if 255 in img34[:150,:] and (dudoan==0 or dudoan==1):
                                lm=False
                            else:
                                lm=True
                            Control(0,0)
               ...
               # print(arrmin, arrmax)
                #print(uncheck)
                if (255 in img4 or 255 in img5) and (w>30 and h>30):
                    if dudoan==5:
                        Control(5,0)
                    else:
                        Control(0,0)
                ...
                if (arrmin3!=-1 and arrmax3!=-1):
                    ...
                    if abs(arrmin3 - img.shape[1] / 2)>abs(arrmax3 - img.shape[1] / 2):
                        bias=10
                        mid = int((arrmax + arrmin) / 2)
                        angle = math.degrees(math.atan((mid - img.shape[1] / 2) / (img.shape[0] - 50)))
                        Control(pid(angle)+ bias,speed)
                    else:
                        bias=-10
                        mid = int((arrmax + arrmin) / 2)
                        angle = math.degrees(math.atan((mid - img.shape[1] / 2) / (img.shape[0] - 50)))
